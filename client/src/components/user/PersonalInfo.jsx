...
import { Edit2, ShieldCheck, Mail, Phone, User, Wallet, Users, Lock, ArrowRight } from 'lucide-react';

import EditProfileModal from './EditProfileModal';
import { updateProfile, getUserMe } from '../../api/user/user.api'; // 🟢 Using your getUserMe function
    ...
    const handleUpdateProfile = async (data) => {
        try {
            ...
            if (response.data.requiresVerification) {
                ...
                dispatch(setUser(response.data.user));
                nxToast.success("Profile Info Updated", "Your Profile info has been successfully updated.");
        ...
        <div className="space-y-6 animate-in fade-in duration-700">
            ...
            <div className={`${glassStyle} p-10`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <ProfileField label="Verification Status" value={user?.isVerified ? "Verified" : "Active"} icon={<ShieldCheck size={14} />} isStatus />
                </div>
            </div>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                <div onClick={() => navigate('/profile/wallet')} className="group cursor-pointer bg-gradient-to-br from-white/[0.08] to-transparent backdrop-blur-xl border border-white/10 rounded-[2rem] p-8 shadow-xl hover:border-[#7a6af6]/40 transition-all relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-start mb-3">
                        <p className="flex items-center gap-2 text-[9px] font-black uppercase text-[#7a6af6] tracking-[0.4em]"><Wallet size={16} /> Total Credits</p>
                    ...
                    <button className="text-[8px] font-black uppercase tracking-widest text-white/40 group-hover:text-white border border-white/10 group-hover:border-[#7a6af6] px-3 py-1 rounded-lg transition-all">View Ledger</button>
                </div>
...
const StatCard = ({ title, value, icon }) => (
    <div className="bg-gradient-to-br from-white/[0.08] to-transparent backdrop-blur-xl border border-white/10 rounded-[2rem] p-8 shadow-xl">
        <p className="flex items-center gap-2 text-[9px] font-black uppercase text-[#7a6af6] tracking-[0.4em] mb-3">{icon} {title}</p>

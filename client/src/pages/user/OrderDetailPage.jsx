...
import {
    ...
    ArrowDownLeft, RefreshCw, Wallet, Clock
} from 'lucide-react';
...
const OrderDetailPage = () => {
    ...
    const financials = useMemo(() => {
        ...
        const isShipped = ['shipped', 'out_for_delivery', 'delivered', 'returned'].includes(order.status.toLowerCase());
        ...
        order.items.forEach(item => {
            if (['Cancelled', 'Returned'].includes(item.status)) {
                itemsRefundTotal += (item.totalAmount || 0);
                cancelledCount++;
            }
        });

        const allItemsSettled = cancelledCount === order.items.length;
        
        // Final refund only includes delivery if canceled BEFORE shipping AND all items are canceled
        const finalRefundValue = (allItemsSettled && !isShipped) 
            ? (itemsRefundTotal + delivery) 
            : itemsRefundTotal;
        ...
        return {
            ...
            // 🟢 FIXED: Never set net to 0. Always show the Master Total for reference.
            net: masterTotal 
        };
    }, [order]);
    ...
    const handleRetryPayment = async () => {
        try {
            ...
            const { data: sessionData } = await userAxios.post("/user/payment/create-order", {
                items: order.items,
                totals: { deliveryCharge: order.deliveryCharge, totalDiscount: order.totalDiscount }
            });
        ...
        } catch (err) { nxToast.error("Payment Error"); }
    };
        ...
        <div className="w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between pb-8 border-b border-white/5">
                ...
                {['pending', 'processing', 'confirmed'].includes(order.status.toLowerCase()) && order.items?.some(i => i.status !== 'Cancelled') && (
                    <button onClick={() => openModal('cancel_all')} className="px-6 py-3 rounded-2xl border border-red-500/30 bg-red-500/10 text-red-500 text-[10px] font-black uppercase tracking-widest hover:bg-red-500 hover:text-white transition-all">
                        Cancel Order
                    </button>
                )}
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
                <div className="xl:col-span-8 space-y-8">
                    <div className={`${glassStyle} p-10`}>
                        ...
                        {order.status === 'payment_failed' ? (
                            <div className="py-10 border-2 border-dashed border-red-500/20 rounded-3xl flex flex-col items-center justify-center gap-2 text-red-400/40"><XCircle size={32} /><p className="text-[10px] font-black uppercase tracking-widest">Payment Incomplete</p></div>
                        ) : (
                    ...
                    <div className="space-y-4">
                        <h2 className="text-[9px] font-black uppercase text-[#7a6af6] italic tracking-[0.3em] px-2 flex items-center gap-2"><Box size={14} /> Item Details</h2>
                                ...
                                <div className="flex-1 w-full space-y-2">
                                    <div className="flex justify-between items-start">
                                        ...
                                        <div className="text-right">
                                            ...
                                            {((item.status === 'Returned') || (item.status === 'Cancelled' && order.paymentStatus === 'Paid')) && (
                                                <div className="flex items-center gap-1 text-[7px] font-black text-green-400 uppercase tracking-tighter mt-1 bg-green-400/10 px-1.5 py-0.5 rounded border border-green-400/20">
                                                    <Wallet size={8} /> Refunded
                                                </div>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-4 mt-2">
                                        ...
                                        {['pending', 'processing', 'confirmed'].includes(order.status.toLowerCase()) && item.status?.toLowerCase() === 'placed' && (
                                            <button onClick={() => openModal('cancel', item)} className="text-[7px] font-black uppercase text-red-500 hover:text-red-400 flex items-center gap-1 transition-all"><XCircle size={10} /> Cancel Item</button>
                                        )}
                                        {order.status.toLowerCase() === 'delivered' && item.status?.toLowerCase() === 'delivered' && (
                                            <button onClick={() => openModal('return', item)} className="text-[7px] font-black uppercase text-amber-500 hover:text-amber-400 flex items-center gap-1 transition-all"><RotateCcw size={10} /> Return Item</button>
                                        )}
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className={`${glassStyle} p-8`}>
                        <h2 className="text-[10px] font-black uppercase text-[#7a6af6] italic tracking-[0.3em] mb-6 flex items-center gap-2"><MapPin size={14} /> Delivery Address</h2>
                ...
                <aside className="xl:col-span-4 space-y-6">
                    <div className="bg-white text-black p-10 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
                        <div className="absolute top-0 right-0 p-4 opacity-[0.03] transition-opacity pointer-events-none"><ShieldCheck size={160} /></div>
                        ...
                        <div className="space-y-4 text-[11px] font-bold uppercase tracking-tight relative z-10">
                            ...
                            {financials.savings > 0 && (
                                <div className="text-[9px] text-green-600 font-black italic text-right">
                                    You saved ₹{financials.savings.toLocaleString()} on this order
                                </div>
                            )}

                            {financials.totalRefunded > 0 && (
                                <div className="py-3 px-4 bg-slate-50 border border-slate-100 rounded-xl flex justify-between items-center text-green-600 animate-in slide-in-from-top-2">
                                    <span className="flex items-center gap-1.5 italic font-black text-[9px]"><RotateCcw size={12} strokeWidth={3} /> Total Refunded</span>
                                    <span className="font-black">- ₹{financials.totalRefunded.toLocaleString()}</span>
                                </div>
                            )}
                    ...
                    <div className={`${glassStyle} p-8 flex justify-between items-center`}>
                        <div className="flex flex-col gap-1">
                            <p className="text-[8px] font-black text-white/20 uppercase tracking-widest mb-1 italic flex items-center gap-2">
                                {order.paymentMethod === 'wallet' ? <Wallet size={12} /> : <CreditCard size={12} />}
                                Payment Method
                            </p>
                            <p className="text-xs font-black uppercase italic text-white">
                                {order.paymentMethod === 'cashOnDelivery' ? 'Cash on Delivery' : order.paymentMethod === 'wallet' ? 'Digital Wallet' : 'Razorpay'}
                            </p>
                            <p className={`text-[8px] font-black uppercase mt-1 ${['Paid', 'Refunded', 'refunded'].includes(order.paymentStatus) ? 'text-green-500' : 'text-amber-500'}`}>
                                {order.paymentStatus}
                            </p>
                        </div>
                        <div className={`p-2 rounded-xl bg-white/5 border border-white/10 ${['Paid', 'Refunded', 'refunded'].includes(order.paymentStatus) ? 'text-green-500' : 'text-amber-500'}`}>

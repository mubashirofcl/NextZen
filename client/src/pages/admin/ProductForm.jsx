...
import {
    ...
    Activity, ClipboardList, Trash2, Ruler, Maximize, BoxSelect
} from "lucide-react";
...
const ProductForm = () => {
    ...
    const onFormSubmit = async (payload) => {
        ...
        try {
            isEditMode ? await updateMutation.mutateAsync({ id, ...cleanedPayload }) : await createMutation.mutateAsync(cleanedPayload);
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar pr-1">
                    <form className="space-y-3 pb-8" onSubmit={(e) => e.preventDefault()}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                ...
                                <div className="md:col-span-2 space-y-3 border-t pt-4">
                                    <div className="flex justify-between items-center px-1">
                                        <div className="flex items-center gap-2"><ListChecks size={14} className="text-[#7a6af6]" /><label className="text-[9px] font-bold text-[#0F172A] uppercase tracking-widest">Highlights</label></div>
                                        <button type="button" onClick={() => appendHighlight("")} className="text-[8px] font-black text-[#7a6af6] bg-purple-50 px-2 py-0.5 rounded-full">+ New Entry</button>
                                    </div>
                                    <div className="grid md:grid-cols-2 gap-3">
                                        {highlightFields.map((field, index) => (
                                            <div key={field.id} className="flex gap-2">
                                                <input {...register(`highlights.${index}`, { required: true })} className="flex-1 bg-white border-2 border-slate-100 rounded-lg px-4 py-2 text-[11px] font-bold focus:border-[#7a6af6] outline-none" />
                                                <button type="button" onClick={() => removeHighlight(index)} className="p-1.5 text-slate-300 hover:text-red-500 transition-all"><Trash2 size={16} /></button>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                        ...
                        <div className="flex justify-between items-center px-6 py-3 bg-[#0F172A] rounded-[16px] text-white">
                            ...
                            {/* FIXED BUTTON: Now includes sizes array when appending */}
                            <button type="button" onClick={() => appendVariant({ color: "New Color", hex: "#000000", images: [], sizes: getNewVariantSizes() })} className="px-4 py-1.5 bg-[#7a6af6] rounded-lg text-[9px] font-black uppercase tracking-widest">+ Add Colorway</button>
                        </div>
                        ...
                        <div className="bg-white rounded-[20px] p-6 shadow-sm border border-slate-200 mt-4">
                            <div className="flex items-center gap-2 mb-5 border-b border-slate-50 pb-3">
                                ...
                                <h3 className="text-[10px] font-black text-[#0F172A] uppercase tracking-widest">Deployment Summary</h3>
                            </div>
